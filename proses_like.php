...
while ($row_user = mysqli_fetch_assoc($log_user)) { //perulangan untuk menampilkan data
    if ($row_user['username'] == $username) { //jika username yang ada pada database sama dengan username pengguna yang sudah login
        $id_user = $row_user['id_user']; //jika id user sama
        $sql = mysqli_query($koneksi, "INSERT INTO tb_rating (id_artikel, id_user, rating) VALUES ('$id','$id_user', '1')"); //membuat variabel $sql untuk menyimpan query insert
